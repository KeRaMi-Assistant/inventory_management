#!/usr/bin/env bash
# uninstall-analyzer.sh — Stops + removes the Analyzer LaunchAgent.
#
...
# ---------------------------------------------------------------------------
# Best-effort: kill any lingering analyzer.sh processes
# ---------------------------------------------------------------------------
pkill -f "analyzer.sh" 2>/dev/null || true
...
if command -v audit_record >/dev/null 2>&1; then
  audit_record "uninstall-analyzer" "info" "LaunchAgent uninstalled" \
...
if [ -x "$NOTIFY_SH" ]; then
  REPO_ROOT="$ROOT" "$NOTIFY_SH" info "claude-code" \
    "Analyzer LaunchAgent uninstalled" \
