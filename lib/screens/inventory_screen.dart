...
class _InventoryScreenState extends State<InventoryScreen>
  ...
  Widget _buildHeader(BuildContext context, InventoryProvider provider, bool isNarrow, NumberFormat money, double width) {
    final l10n = AppLocalizations.of(context);
    final kpis = [
      _kpi(l10n.inventoryKpiTotalItems, '${provider.inventoryItems.length}', Icons.category_outlined, AppTheme.accentTextOf(context)),
      _kpi(l10n.inventoryKpiTotalStock, '${provider.totalStockQuantity}', Icons.inventory_2_outlined, AppTheme.successTextOf(context)),
      _kpi(l10n.inventoryKpiCriticalItems, '${provider.criticalStockCount}', Icons.warning_amber_rounded, AppTheme.dangerTextOf(context)),
      _kpi(l10n.inventoryKpiStockValue, money.format(provider.totalStockValue), Icons.euro_outlined, AppTheme.warningTextOf(context)),
    ];
                    ...
                    Builder(builder: (ctx) {
                      ...
                      return Row(children: [
                        TextButton.icon(
                          ...
                          icon: Icon(Icons.add_circle_outline, size: 16, color: AppTheme.successTextOf(ctx)),
                          label: Text(l10n.inventoryStockIn, style: TextStyle(color: AppTheme.successTextOf(ctx), fontSize: 12)),
                          style: TextButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 6)),
                        ),
                        TextButton.icon(
                          onPressed: () => _adjust(context, provider, item, false),
                          icon: Icon(Icons.remove_circle_outline, size: 16, color: AppTheme.warningTextOf(ctx)),
                          label: Text(l10n.inventoryStockOut, style: TextStyle(color: AppTheme.warningTextOf(ctx), fontSize: 12)),
                          style: TextButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 6)),
                        ),
                        IconButton(
                        ...
                        IconButton(
                          ...
                          icon: Icon(Icons.delete_outline, size: 18, color: AppTheme.dangerTextOf(ctx)),
                        ),
                      ]);
                    }),
                  ],
        ...
        DataCell(Row(
          children: [
            IconButton(
              ...
              onPressed: () => _adjust(context, provider, item, true),
              icon: Icon(Icons.add_circle_outline, size: 18, color: AppTheme.successTextOf(context)),
            ),
            IconButton(
              tooltip: l10n.inventoryStockOutTooltip,
              onPressed: () => _adjust(context, provider, item, false),
              icon: Icon(Icons.remove_circle_outline, size: 18, color: AppTheme.warningTextOf(context)),
            ),
            IconButton(
              tooltip: l10n.actionEdit,
              onPressed: () => showDialog(
